...
- **Fix:** A `package-lock.json` is generated by running `npm install` once locally and committing the lockfile. The Dockerfile falls back to `npm install` if the lockfile is absent, but committing the lockfile is the correct production practice.
